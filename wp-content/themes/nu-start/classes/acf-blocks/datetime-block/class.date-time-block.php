...
class NU_DateTime_Helper
{
	...
	public static function build_datetime_return_string(){
		...
		$location = !empty($fields['location']) ? '<p class="nu__datetime-location"><span>' . $fields['location'] . '</span></p>' : '';
		...
		if( !empty($fields['spans_multiple_days']) ){
			...
			$return .= sprintf(
				$guides['multiple-days']
				,!empty($startsAt) || !empty($endsAt) ? '<p class="nu__datetime-times">'.$startsAt . $endsAt . '</p>' : ''
				,!empty($startsOn) || !empty($endsOn) ? '<p class="nu__datetime-dates">'.$startsOn .' - '. $endsOn . '</p>' : ''
			);
			...
			$happensOn = !empty( $fields['one_day']['happens_on'] ) ? '<p class="nu__datetime-dates"><span class="nu__datetime-startsat">'. DateTime::createFromFormat('Ymd', $fields['one_day']['happens_on'] )->format($date_format) . '</span></p>' : '';
