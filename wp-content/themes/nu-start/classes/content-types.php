...
class NU__ContentTypes
{
	...
	{
		// 
		$parentContentTypes = get_dir_filenames_and_paths(get_template_directory() . "/classes/content-types/enabled/");
		// 
		$registration_files = $parentContentTypes;

		// 
		if (is_child_theme() && is_dir(get_stylesheet_directory() . '/classes/content-types/enabled/')) {
			$childContentTypes = get_dir_filenames_and_paths(get_stylesheet_directory() . '/classes/content-types/enabled/');
			$registration_files = array_merge($parentContentTypes, $childContentTypes);
		}
		// 
		foreach ($registration_files as $name => $path) {
			include_once($path);
		}
	...
	public static function _register_custom_post_type($literal = '', $name = '', $singular = '', $rewrite = '',  $hierarchical = false, $dashicon = '')
	{
		...
		if (!post_type_exists(sanitize_title('nu_' . $literal))) {
			...
			$custom_tax_mb = new Taxonomy_Single_Term('nu_profiles-types', 'nu_profiles');
		...
		foreach (NU__ContentTypes::$activeTypes as $activeType) {
			...
			if ($activeType == $post_type) {
				...
				foreach ($taxonomies as $taxonomy_slug) {
					...
					$taxonomy_name = !empty($taxonomy_obj->labels->name) ? $taxonomy_obj->labels->name : '';
...
// add_action('after_setup_theme', 'NU__ContentTypes::_init' );
