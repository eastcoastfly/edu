...
class Gutenslider {
  ...
  initPremium() {
    if (typeof (simpleParallax_min_default()) !== undefined) {
      const parallaxSliders = document.querySelectorAll('.wp-block-eedee-block-gutenslider.has-parallax');
      parallaxSliders.forEach(function (el) {
        const orientation = el.dataset.parallaxDirection || 'top';
        const scale = el.dataset.parallaxAmount || 1.3;
        const images = el.querySelectorAll('.eedee-background-div img');
        const lax = new (simpleParallax_min_default())(images, {
          scale,
          orientation
        });
      });
